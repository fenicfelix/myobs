						...
						<tbody>
							...
							if($items) {
								...
								foreach($items as $item) {
									...
									<tr class="gradeX">
										<td><?=$i;?></td>
										<td><img src="<?=base_url().'uploads/playlists/'.$item->image_url;?>" alt="<?=$item->title;?> image" width="50px"/></td>
										<td><?=$item->title;?></td>
										<td>
											<?php
												if($item->stage_name) echo $item->stage_name;
												else echo $item->first_name." ".$item->last_name;
											?>
										</td>
										<td><?=number_format($item->no_of_items);?></td>
										<td class="actions">
											<?php if($is_read) { ?>
												<a href="<?=site_url("admin/".$this->uri->segment(2)."/read/".$item->playlist_id);?>" class="edit_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" role="button">
											...
											<?php if($is_edit) { ?>
												<a href="<?=site_url("admin/".$this->uri->segment(2)."/edit/".$item->playlist_id);?>" class="edit_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" role="button">
													<span class="ui-button-icon-primary ui-icon ui-icon-pencil"></span>
													<span class="label label-primary"><i class="fa fa-edit"></i>&nbsp;Edit</span>
												</a>
											<?php } ?>
											<?php if($is_delete) { ?>
												<a onclick="javascript: return delete_row('<?=site_url("admin/".$this->uri->segment(2)."/delete/".$item->playlist_id);?>', '<?=$i;?>')" href="javascript:void(0)" class="delete_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" role="button">
