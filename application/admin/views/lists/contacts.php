	...
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading clearfix">
				...
				<ul class="panel-tool-options">
					...
					if($this->input->get('term')) {
						$download_url = site_url("admin/contacts_xls/?term=".$this->input->get('term'));
						?>
			...
			<div class="panel-body">
							...
							if($contacts) {
								...
								foreach($contacts as $contact) {
									...
									<tr class="gradeX">
										...
										<td><?=$contact->province_name;?></td>
										<td class="actions">
											<a href="<?=site_url("admin/".$this->uri->segment(2)."/read/".$contact->contact_id);?>" class="edit_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" role="button">
												<span class="ui-button-icon-primary ui-icon ui-icon-document"></span>
												<span class="label label-warning"><i class="fa fa-eye"></i>&nbsp;View</span>
											</a>
				
											<a href="<?=site_url("admin/".$this->uri->segment(2)."/edit/".$contact->contact_id);?>" class="edit_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" role="button">
												<span class="ui-button-icon-primary ui-icon ui-icon-pencil"></span>
												<span class="label label-primary"><i class="fa fa-edit"></i>&nbsp;Edit</span>
											</a>
											<!-- a onclick="javascript: return delete_row('<?=site_url("admin/".$this->uri->segment(2)."/delete/".$contact->contact_id);?>', '<?=$i;?>')" href="javascript:void(0)" class="delete_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary" role="button">
				...
				<div>
					<p>
						...
						if($all_contacts) {
							...
							if($all_contacts->total > $num_rec_per_page*$page) $end_row = (($start_row+$num_rec_per_page)-1);
							else $end_row = $all_contacts->total;
							$total_items = $all_contacts->total;
						}
				...
				<div class="">
					<?php
						$total_records = $all_contacts->total;  //count number of records
